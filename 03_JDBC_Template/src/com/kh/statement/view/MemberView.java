...
public class MemberView {
	...
	public void mainMenu() {
		 ...
		 *
		 * 나만의 테이블에 INSERT 전체 조회 유니크제약조건 걸려있는 컬럼으로 조회 LIKE키워드 써서 조회하는거
		 *
		 * --> 요게 쫌 부담인데..?
		 *
		 * 2번 --> PLANT테이블만들기 한걸로
	 ...
	 * 
	 * 컬럼에 INSERT할 값들을 모두 입력받은 후 입력받은 값을 전부 컨트롤러로 전달
	 */
	private void save() {
	...
	private void findAll() {
		...
		if (members.isEmpty()) {
			...
			for (Member member : members) {
				System.out.println("======================");
				System.out.print(member.getUserNo() + "번 회원의 정보");
	...
	private void findById() {
		...
		/*
		 * 자바에서 값의 종류 == 자료형 정수 = bytem short, int, long 실수 = float, double 문자 = char
		 * 논리값 = boolean
		 * 
		 * 주소값 = 나머지 다
		 */
